...
def _available_resources_per_node_fallback():
    """
    Try old private API first; if missing (new Ray), fall back to a single-node view
    using public APIs. This is sufficient for single-node training.
    """
    import ray
    # 旧版 Ray（如果存在就直接用）
    try:
        return ray.state.available_resources_per_node()
    except Exception:
        pass

    # 新版 Ray：用公共 API 做回退（单机足够用；多机可自行扩展为聚合）
    avail = ray.available_resources()  # 公共 API：集群层面的可用资源
    try:
        node_id = ray.get_runtime_context().get_node_id()  # 公共 API：当前节点 ID
    except Exception:
        node_id = "local_node"
    return {node_id: dict(avail)}
...
class ResourcePoolManager:
    ...
    def _check_resource_available(self):
        ...
        node_available_resources = _available_resources_per_node_fallback()
